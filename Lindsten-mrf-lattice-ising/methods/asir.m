function [X,lZ,logW,ess_log] = asir(X,Jv0,Jh0,H0,Jv1,Jh1,H1,alpha_vec,par)
% Input:
%   X, M x M x N: initial samples
%   Jv0, Jh0, H0 M x M (or scalar): initial edge/local interactions
%   Jv1, Jh1, H1 M x M: final edge/local interactions
...
for iAlpha = 1:numAlpha
    ...
    if(iAlpha > 1) % Don't propagate on the first iteration
        X = gibbsgrid(X, alpha_vec(iAlpha)*Jvt + Jv0, alpha_vec(iAlpha)*Jht + Jh0, alpha_vec(iAlpha)*Ht + H0);
    end
    
    %%% Weight computation    
    % Computes sum_{ij} J_{ij}*x_i*x_j for every page in X
    S = sum_diff_x(X, Jvt, Jht, Ht);
    % Compute new weights
