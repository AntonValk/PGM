function [X,lZ,logW,alpha_log,ess_log] = asir_adpt(X,Jv0,Jh0,H0,Jv1,Jh1,H1,alpha_init,par)
% Input:
...
this_is_asir = strcmpi(par.method,'asir');
while(alpha < 1)
    if(par.printOn && this_is_asir)
        fprintf('alpha = %2.6f\n',alpha);
    end
    
    %%% Propagate from alpha --> alpha_new
    if(k > 1) % Don't propagate on the first iteration
         X = gibbsgrid(X, alpha*Jvt + Jv0, alpha*Jht + Jh0, alpha*Ht + H0);
    end
